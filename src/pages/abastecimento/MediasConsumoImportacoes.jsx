...
export default function MediasConsumoImportacoes() {
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      const header = raw[0] || [];
      const idx = {};
      for (const [campo, al] of Object.entries(MAPA)) {
        idx[campo] = header.findIndex(h => al.includes(norm(h)));
      }
      ...
      <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:12, padding:'14px 20px', marginBottom:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ flex:1 }}>
            ...
            <p style={{ fontSize:11, color:'#9ca3af', margin:0 }}>Somente administradores • Gerencie arquivos importados</p>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap', marginBottom: preview.frota ? 0 : 12 }}>
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:8 }}>
              ...
              <button onClick={salvarImportacao} disabled={salvando || !preview.frota || !preview.nomesConfirmados}
                style={{ padding:'7px 16px', border:'none', borderRadius:8, background: (preview.frota && preview.nomesConfirmados) ? '#16a34a' : '#9ca3af', color:'#fff', fontSize:12, fontWeight:600, cursor: (preview.frota && preview.nomesConfirmados) ? 'pointer' : 'not-allowed' }}>
              ...
              {(() => {
                ...
                return (
                  <span style={{ fontSize:12, color:'#15803d' }}>
                    <i className="ti ti-check"></i> {total} motoristas verificados — {exatos} com match
                    {aliasOk > 0 ? `, ${aliasOk} por alias` : ''}
                    {semMatch > 0 ? `, ${semMatch} sem correspondência` : ''}
                  </span>
                );
              })()}
              ...
              {importacoes.map((im, i) => (
                <tr key={im.id} style={{ borderBottom:'1px solid #f3f4f6', background: i%2===0?'#fff':'#fafafa' }}>
                  ...
                  <td style={{ padding:'10px 16px' }}>
                    <span style={{ padding:'2px 10px', borderRadius:20, background: im.frota==='BAÚ'?'#eff6ff':'#f0fdf4', color: im.frota==='BAÚ'?'#1d4ed8':'#15803d', fontSize:11, fontWeight:700, border:`1px solid ${im.frota==='BAÚ'?'#bfdbfe':'#86efac'}` }}>
                      {im.frota || 'Geral'}
                    </span>
                  </td>
